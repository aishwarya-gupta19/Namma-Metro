...
void train();
void run();
int ch=0,tran=0,movemany=0,farx=750,fary=750,movemanx=0;
float s=0,open_door1=0,open_door2=0;//tran s and farx fary for tranlating train in side,front & far view
bool stop_front=false,stop_far=false,stop_side=false,stopman=false;
...
void station()
{
	...
	glEnd(); //end of platform back
	   ...
	   glBegin(GL_POLYGON);
		...
		//person 1
		glPushMatrix();
		glTranslatef(movemanx,100+movemany,0);
		glPushMatrix();
		glColor3f(0,0,0);
